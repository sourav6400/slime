...
<script>
    ...
    $(document).ready(function () {
        ...
        dataTable = $('#example').DataTable({
            ...
            columns: [
                ...
                
                {
                    "data": function (data) {
                        if (data.dp_url != null) {
                            ...
                            '<br>'+
                            '<span><a onclick="viewProfile('+ data.id +')" class="badge badge-secondary" style="float: left;">'+ data.name +'</a></span>';
                            // '<a class="badge rounded-pill badge-light-success link-bottom" target="_blank" href="' + data.telegram + '" title="Contact"><i class="fa fa-address-book" aria-hidden="true"></i></a>'
                        }
                        else {
                ...
                {
                    "data": function (data) {
                        return data.total_follower.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
                ...
                @if ($influencer_permission != null && $influencer_permission -> edit == 1) {
                "data": function(data) {
                    if (data.contact_type == 'telegram' && data.contact != null) {
                        return '<a class="me-1" type="button" onclick="quickEdit(' + data.id + ' )"><i class="fas fa-edit" title="Quick Edit"></i></a>' +
                        '<a class="delete-influencer me-1" data-bs-toggle="modal" data-bs-target="#deletePermissionModal" data-influencer_id="' + data.id + '"><i class="fa fa-trash" aria-hidden="true" title="Remove"></i></a>' +
                        '<a target="_blank" href="https://t.me/' + data.contact + '" title="Contact"><i class="fa fa-address-book" aria-hidden="true"></i></a>';
                    }
                    else{
                        return '<a class="me-1" type="button" onclick="quickEdit(' + data.id + ' )"><i class="fas fa-edit" title="Quick Edit"></i></a>' +
                        '<a class="delete-influencer me-1" data-bs-toggle="modal" data-bs-target="#deletePermissionModal" data-influencer_id="' + data.id + '"><i class="fa fa-trash" aria-hidden="true" title="Remove"></i></a>';
    ...
    function viewProfile(id) {
        ...
        var url = "{{ route('influencer_connections', ":id") }}";
